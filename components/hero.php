...
<section class="relative py-20 lg:py-0 h-auto lg:h-[92vh] min-h-[720px] w-full overflow-hidden bg-slate-50 flex items-center">
    ...
    <div class="relative w-full h-full flex items-center" id="hero-carousel">
        ...
        <div class="carousel-slide absolute inset-0 w-full h-full <?php echo $isActive ? 'opacity-100 z-10' : 'opacity-0 pointer-events-none z-0'; ?> transition-opacity duration-1000 ease-in-out flex items-center" data-index="<?php echo $slideIdx; ?>">
            ...
            <!-- Slide White Overlay -->
            <div class="absolute inset-0 bg-gradient-to-r from-white via-white/92 to-white/35 lg:to-transparent"></div>
            
            <!-- Slide Content -->
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-20">
                <div class="max-w-4xl space-y-6 md:space-y-8 animate-fade-in-up">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold bg-brand-550/10 border border-brand-500/20 text-brand-700 tracking-wider uppercase">
                        <i class="fa-solid fa-rocket mr-1"></i> <?php echo htmlspecialchars($slide['badge']); ?>
                    </span>
                    <!-- Larger Main Header -->
                    ...
                    <!-- Points Layout: Large Icon, Heading + Subcontent -->
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-3xl pt-2">
                        <!-- Point 1 -->
                    ...
                    <div class="flex items-center gap-3 pt-2">
                        <a href="<?php echo htmlspecialchars($slide['btn1_url']); ?>" class="px-7 py-3.5 rounded-full text-xs sm:text-sm font-bold text-white accent-gradient hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5">
                            <?php echo htmlspecialchars($slide['btn1_text']); ?>
                        </a>
                        <a href="<?php echo htmlspecialchars($slide['btn2_url']); ?>" class="px-7 py-3.5 rounded-full text-xs sm:text-sm font-bold text-slate-600 bg-white border border-slate-200 shadow-sm hover:bg-slate-50 transition-all duration-300">
    ...
    <!-- Carousel Controls -->
    <div class="absolute bottom-24 right-8 z-30 flex items-center gap-3">
        <!-- Prev Button -->
    ...
    <div class="absolute bottom-24 left-8 z-30 flex items-center gap-2" id="carousel-indicators">
